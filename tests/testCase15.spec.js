...
test('Place Order: Register while Checkout', async ({ page }) => {
    ...
    // 11. Verifica que los datos de la dirección de facturación sean correctos
    await checkAddresandOrder(page);

    // 12. Escribe descripcion y click al boton "Place Order"
    await page.fill('textarea[name="message"]', 'Descripcion de test');
    await clickButton(page, 'text="Place Order"');

    // 13. Ingresa los datos de la tarjeta de crédito
    await insertCreditCard(page);

    // 14. Click al boton "Pay and Confirm Order"
    await clickButton(page, 'text="Pay and Confirm Order"');

    // 15. Verifica que la orden fue realizada correctamente
    await verifyVisibility(page, 'h2:has-text("Order Placed!")');

    // 16. Borra la cuenta
    await clickButton(page, 'a:has-text("Delete Account")');

    // 17. Verifica que se haya borrado la cuenta y click al boton "Continue"
